...
export default function Dashboard() {
  ...
  try {
      ...
      console.log('Dashboard state:', { hasToken: !!token, userEmail: user?.email });

      const fetchProjects = async () => {
        console.log('fetchProjects called');
        try {
          const res = await axios.get(`${API_URL}/projects`, { headers: { Authorization: `Bearer ${token}` } });
          console.log('API Result:', res.data.length, 'projects');
      ...
      if (loading) return <div style={{ background: '#1e1e1e', height: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#888' }}><RefreshCw className="animate-spin" /></div>;

      return (
        <div style={{ display: 'flex', flexDirection: 'column', minHeight: '100vh', width: '100vw', background: '#0f0f0f', color: 'white' }}>
            <nav style={{ padding: '12px 40px', background: '#181818', borderBottom: '1px solid #282828', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}><div style={{ background: '#0071e3', padding: '6px', borderRadius: '8px' }}><FileText size={18}/></div><span style={{ fontWeight: 800, fontSize: '18px' }}>LaTeX Workshop</span></div>
            ...
            <main style={{ flex: 1, padding: '40px 80px', maxWidth: '1200px', margin: '0 auto', width: '100%' }}>
                ...
                <div style={{ background: '#181818', borderRadius: '12px', border: '1px solid #282828', overflow: 'hidden' }}>
                    <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                        <thead><tr style={{ background: '#1e1e1e', color: '#666', fontSize: '11px', textTransform: 'uppercase' }}><th style={{ padding: '12px 20px', textAlign: 'left' }}>Name</th><th style={{ padding: '12px 20px', textAlign: 'left' }}>Date</th><th style={{ width: '50px' }}></th></tr></thead>
                        <tbody>
                            {filteredProjects.map((p: any) => (
                                <tr key={p._id} onClick={() => navigate(`/project/${p._id}`)} style={{ borderBottom: '1px solid #222', cursor: 'pointer' }}>
                                    <td style={{ padding: '16px 20px' }}>{p.name}</td>
                                    <td style={{ padding: '16px 20px', color: '#666' }}>{new Date(p.lastModified).toLocaleDateString()}</td>
                                    <td><ExternalLink size={14} color="#333"/></td>
                                </tr>
                            ))}
                        </tbody>
                    </table>
      ...
      return <div style={{ color: 'red', padding: '40px' }}>Dashboard Crash: {(e as any).message}</div>;
  }
}
